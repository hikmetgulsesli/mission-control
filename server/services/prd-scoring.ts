...
export function scorePrd(content: string): ScoreDetails {
  ...
  // 1. Page and screen detail (0-20)
  let pageDetail = 0;
  // Count pages from page/screen/view headings or route paths like (/path).
  const screenHeaders = lines.filter(l =>
    /^#{1,3}\s.*(page|screen|view)/i.test(l) ||
    /^#{1,3}\s+.*\(\/[^)]*\)/.test(l) ||
    /^#{1,3}\s+.*\(\/\)/.test(l)
  ).length;
  const hasRoutes = /route|path|navigation|\(\/[a-z]/i.test(lower);
  ...
  else { pageDetail = 2; missing.push('Add page and screen details'); }
  if (hasRoutes) pageDetail = Math.min(20, pageDetail + 3);
  if (!hasPageDescriptions) missing.push('Add detailed descriptions for each page');

  // 2. Design system (0-20)
  let designSystem = 0;
  const hasColors = /color|palette|#[0-9a-f]{3,6}/i.test(lower);
  const hasFonts = /font|typography/i.test(lower);
  const hasSpacing = /spacing|padding|margin|gap/i.test(lower);
  const hasTheme = /theme|dark|light/i.test(lower);
  if (hasColors) designSystem += 6;
  else missing.push('Define a color palette');
  if (hasFonts) designSystem += 5;
  else missing.push('Add font and typography details');
  if (hasSpacing) designSystem += 4;
  if (hasTheme) designSystem += 5;
  else missing.push('Specify theme details such as dark or light mode');

  // 3. Component definitions (0-15)
  let components = 0;
  const componentKeywords = ['button', 'card', 'modal', 'dialog', 'input', 'form', 'header', 'footer', 'sidebar', 'nav', 'table', 'list', 'tab', 'dropdown', 'tooltip'];
  ...
  else { components = 0; missing.push('Add UI component definitions'); }

  // 4. Animation and transition detail (0-10)
  let animations = 0;
  const hasAnimations = /animation|transition|hover|easing|duration|timing/i.test(lower);
  ...
  else { animations = 0; missing.push('Define animation and transition effects'); }

  // 5. Responsive definition (0-10)
  let responsive = 0;
  const hasBreakpoints = /breakpoint|responsive|mobile|tablet|desktop|media\s*query/i.test(lower);
  const hasGridInfo = /grid|flex|layout|column/i.test(lower);
  if (hasBreakpoints && hasGridInfo) responsive = 10;
  else if (hasBreakpoints || hasGridInfo) responsive = 5;
  else { responsive = 0; missing.push('Specify responsive design rules'); }

  // 6. Data model (0-10)
  let dataModel = 0;
  const hasApi = /api|endpoint|data|fetch|backend|database/i.test(lower);
  const hasModels = /model|schema|table|field|type|interface/i.test(lower);
  const hasState = /state|store|context/i.test(lower);
  if (hasApi) dataModel += 4;
  if (hasModels) dataModel += 3;
  if (hasState) dataModel += 3;
  if (dataModel === 0) missing.push('Add data model and API definitions');

  // 7. Screen count coverage (0-15)
  let screenCount = 0;
  ...
  else { screenCount = 0; missing.push('Define more screens or pages'); }
...
export function checkScreenCoverage(
  ...
  if (savedPages && savedPages.length > 0) {
    const stopWords = new Set(['the', 'and', 'for', 'with', 'prd', 'page', 'screen', 'view']);
    const covered: string[] = [];
    const missing: string[] = [];
    const normalize = (r: string) => r.replace(/\/+$/, ''); // strip trailing slash

    for (const page of savedPages) {
      // 1. Route-based exact match for new screens with a pageRoute field.
      ...
      // 2. Fuzzy title match for legacy screens without pageRoute.
  ...
  // Strategy 1: find pages from a Pages section with numbered, bullet, or h3 list items.
  // Exit on the next h2 heading after entering.
  let inPagesSection = false;
  for (const line of lines) {
    if (/^##\s+.*pages/i.test(line) && !inPagesSection) { inPagesSection = true; continue; }
    if (inPagesSection && /^##\s/.test(line)) { inPagesSection = false; break; }
    if (inPagesSection) {
      // Numbered list: "1. **Home Page** (`/`) - desc"
  ...
  if (sectionHeadings.length < 2) {
    for (const line of lines) {
      // Match h2 page sections: "## 4. Home Page (`/`)" or "## 4. Home Page (/)".
...
export function extractPages(prdContent: string): { name: string; route: string; description: string }[] {
  ...
  // Find the Pages section and parse numbered or bullet lists.
  let inPagesSection = false;
  for (const line of lines) {
    if (/^##\s+.*pages/i.test(line) && !inPagesSection) { inPagesSection = true; continue; }
