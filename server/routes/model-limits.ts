...
router.get('/model-limits', async (_req, res) => {
  try {
    ...
    const providers: ProviderConfig[] = limitsConfig.providers || [];
    const tokenUsage: any[] = dataFile.tokenUsage || [];
    const tokenUsageToday: any[] = dataFile.tokenUsageToday || [];
    const availableModels: any[] = dataFile.availableModels || [];
    ...
    const result = providers.map(p => {
      ...
      const modelNames = availableModels
        .filter((m: any) => modelIds.includes(m.id))
        .map((m: any) => m.name);

      // Sum usage for this provider's models
      const matchUsage = (list: any[]) => {
