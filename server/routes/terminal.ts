...
function validateCommand(command: string, args: string[]): string | null {
  ...
  if (command === 'cat' || command === 'tail') {
    for (const arg of args) {
      ...
      if (!isSafe) return `Path "${arg}" is not in allowed directories`;
      if (arg.includes('..')) return 'Path traversal not allowed';
    }
  }
  ...
  if (command === 'docker') {
    ...
    if (!safe.includes(args[0])) return `Docker subcommand "${args[0]}" is not allowed`;
  }
...
router.post('/terminal/exec', async (req, res) => {
  try {
    ...
    if (activeCommands > 0) activeCommands--;
    res.status(500).json({ error: err.message });
  }
});
