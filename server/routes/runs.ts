...
function transformRun(r: any) {
  ...
  let storyCount = 0;
  const planStep = steps.find((s: any) => s.step_id === 'plan');
  if (planStep?.output) {
...
router.get('/runs', async (_req, res) => {
  try {
    const raw: any[] = await cached('runs', 15000, getRuns);
...
router.get('/runs/:id/detail', async (req, res) => {
  try {
    const raw: any[] = await getRuns();
    const run = raw.find((r: any) => r.id === req.params.id);
    ...
    // Full step outputs
    const fullSteps = steps.map((s: any) => ({
      id: s.step_id,
      agent: s.agent_id,
      status: s.status,
    ...
    if (planStep?.output) {
      ...
      if (jsonMatch) {
        try { stories = JSON.parse(jsonMatch[1]); } catch {}
      }
    }
    ...
    if (repo && existsSync(repo)) {
      try {
        const log = execSync(
          `cd ${JSON.stringify(repo)} && git log --format='{"hash":"%h","message":"%s","date":"%ci","author":"%an"}' --reverse 2>/dev/null`,
      ...
      try {
        const tree = execSync(
          `cd ${JSON.stringify(repo)} && find . -not -path '*/node_modules/*' -not -path '*/.git/*' -type f 2>/dev/null | sort`,
          { encoding: 'utf-8', timeout: 5000 }
        );
    ...
    if (repo && existsSync(repo) && gitLog.length > 0) {
      try {
        const stat = execSync(
          `cd ${JSON.stringify(repo)} && git log --reverse --stat --format="COMMIT:%h" 2>/dev/null`,
          { encoding: 'utf-8', timeout: 5000 }
        );
        let currentHash = '';
