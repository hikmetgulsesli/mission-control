...
export async function tryAutoFix(runId: string, cause: string, storyId?: string | null): Promise<AutoFixResult> {
  const fixes: Record<string, () => Promise<AutoFixResult>> = {
...
export async function failEntireRun(runId: string, reason: string) {
  const safeId = validateId(runId, 'runId');
...
export async function deleteRun(runId: string, cleanupProject = false) {
  ...
  const runs = await sql`SELECT * FROM runs WHERE id = ${safeId}`;
  const run = runs[0];
  ...
  if (cleanupProject && run?.task) {
    ...
    if (localPath) {
      const otherRuns = await sql`SELECT id, run_number, status, task FROM runs WHERE id != ${safeId}`;
...
export async function deleteRunsByProject(projectName: string): Promise<{ deleted: number; log: string[] }> {
  ...
  const allRuns = await sql`SELECT id, run_number, task FROM runs`;
