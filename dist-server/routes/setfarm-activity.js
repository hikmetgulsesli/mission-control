            ...
            return Promise.all([...running, ...recent].map(async (r) => {
                ...
                try {
                    ...
                    if (stories && stories.length > 0) {
                        const done = r.status === 'completed' ? stories.length : stories.filter((s) => s.status === 'done').length;
...
function findExistingService(_port, slug, repo) {
    try {
        ...
        if (repo) {
            const grepResult = execSync('grep -rl "WorkingDirectory=' + repo + '" /etc/systemd/system/*.service 2>/dev/null || true', { timeout: 3000 }).toString().trim();
            if (grepResult) {
        ...
        const units = execSync('systemctl list-units --type=service --all --no-legend', { timeout: 3000 }).toString();
        ...
        if (needsDeploy && repo) {
            ...
            if (deploy.deployed) {
                ...
                try {
                    ...
                    if (stories && stories.length > 0) {
                        const done = run.status === 'completed' ? stories.length : stories.filter((s) => s.status === 'done').length;
                        updateProjectById(result.project.id, {
