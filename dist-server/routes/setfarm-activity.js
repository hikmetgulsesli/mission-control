    ...
    try {
        const data = await cached('af-pipeline', 10_000, async () => {
            ...
            return Promise.all([...running, ...recent].map(async (r) => {
                let storyProgress = { completed: 0, total: 0, verified: 0, skipped: 0, running: 0, pending: 0, done: 0 };
                try {
                    const stories = await getRunStories(r.id);
                    if (stories && stories.length > 0) {
                        const done = r.status === 'completed' ? stories.length : stories.filter((s) => ['done', 'verified', 'skipped'].includes(s.status)).length;
                        const v = stories.filter((s) => s.status === "verified").length;
                        const sk = stories.filter((s) => s.status === "skipped").length;
                        const ru = stories.filter((s) => s.status === "running").length;
                        const pe = stories.filter((s) => s.status === "pending").length;
                        const dn = stories.filter((s) => s.status === "done").length;
...
router.get('/setfarm/runs/:id/plan', async (req, res) => {
    try {
        ...
        const planStep = (run.steps || []).find((s) => s.step_id === 'plan' && s.status === 'done');
...
function saveSyncState(finished, running) {
    try {
...
async function createBuildingProject(run) {
    ...
    if (!allowedWorkflows.includes(run.workflow_id))
        return;
    if (!run.task)
        return;
...
function extractProjectName(task) {
    ...
    if (buildMatch)
        return stripArticles(buildMatch[1]).replace(/[:\-\u2013]+$/, '').trim();
    // Pattern 2: "ProjectName - feature description" or "ProjectName — description"
    // Split on " - ", " — ", " : " and take only the first part (project name)
...
function slugify(name, maxLen = 30) {
...
function updatePortRegistry(port, name) {
    try {
        let reg = readFileSync(PORT_REGISTRY, 'utf-8');
        if (reg.includes('| ' + port + ' |'))
            return;
        ...
        while (used.has(next) && next <= PORT_RANGE_END)
            next++;
...
function ensureExpressStatic(repo) {
    const serverIndex = join(repo, 'server', 'src', 'index.ts');
    if (!existsSync(serverIndex))
        return;
    let src = readFileSync(serverIndex, 'utf-8');
    if (src.includes('express.static') && src.includes('client/dist'))
...
function patchVitePreview(repo, port) {
    const vitePath = join(repo, 'vite.config.ts');
    if (!existsSync(vitePath))
        return;
    let vite = readFileSync(vitePath, 'utf-8');
    if (vite.includes('preview:') || vite.includes('allowedHosts'))
        return;
    // Add preview block before server block
    vite = vite.replace(/(\s+server\s*:\s*\{)/, `  preview: {\n    port: ${port},\n    host: true,\n    allowedHosts: true,\n  },\n$1`);
    writeFileSync(vitePath, vite);
}
function findExistingService(_port, slug, repo) {
    try {
        // 1. Check by repo WorkingDirectory (most reliable — same dir = same project)
        if (repo && /^[a-zA-Z0-9/_.-]+$/.test(repo)) {
            const grepResult = execFileSync('grep', ['-rl', `WorkingDirectory=${repo}`, '/etc/systemd/system/'], { timeout: 3000, encoding: 'utf-8' }).trim();
            if (grepResult) {
        ...
        const units = execFileSync('systemctl', ['list-units', '--type=service', '--all', '--no-legend'], { timeout: 3000, encoding: 'utf-8' });
...
function autoDeployProject(projectId, projectName, repo, task) {
    if (!repo || !existsSync(repo))
        return { deployed: false, error: 'no repo' };
    ...
    if (!startCmd)
        return { deployed: false, error: 'no start command (build artifacts missing?)' };
    try {
        const unit = [
            '[Unit]',
...
function autoUpdateChecklist(project) {
...
async function syncProjectsFromRuns() {
    ...
    const deployable = allRuns.filter((r) => {
        if (r.status === 'completed')
            return allowedDeployWorkflows.has(r.workflow_id);
    ...
    for (const run of deployable) {
        ...
        const name = extractProjectName(run.task);
        if (!name || name.length < 2) {
            skipped.push(run.id + ': name too short');
            continue;
        }
        let repo = '';
        try {
        ...
        if (needsDeploy && repo) {
            ...
            if (deploy.deployed) {
                ...
                try {
                    ...
                    if (stories && stories.length > 0) {
                        const done = run.status === 'completed' ? stories.length : stories.filter((s) => ['done', 'verified', 'skipped'].includes(s.status)).length;
                        updateProjectById(result.project.id, {
