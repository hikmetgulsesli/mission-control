...
function validatePath(path) {
    ...
    if (!isPathAllowed(resolved))
        return { ok: false, error: 'Access denied: path not in allowed directories', status: 403 };
    if (isBlockedPath(resolved))
        return { ok: false, error: 'Access denied: blocked path', status: 403 };
...
router.put('/files/write', async (req, res) => {
    try {
        ...
        if (buf.length > MAX_FILE_SIZE) {
            res.status(413).json({ error: 'Content too large. Max: 1MB' });
            return;
        }
...
router.post('/files/rename', async (req, res) => {
    try {
        ...
        if (!vOld.ok) {
            res.status(vOld.status).json({ error: vOld.error });
            return;
        }
        const vNew = validatePath(newPath);
        if (!vNew.ok) {
            res.status(vNew.status).json({ error: vNew.error });
            return;
        }
        if (dirname(vOld.resolved) !== dirname(vNew.resolved)) {
            res.status(400).json({ error: 'Rename must stay in the same directory' });
            return;
        }
        if (isRootPath(vOld.resolved)) {
...
router.post('/files/upload', async (req, res) => {
    try {
        ...
        if (!vDir.ok) {
            res.status(vDir.status).json({ error: vDir.error });
            return;
        }
