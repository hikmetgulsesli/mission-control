...
export function updatePortRegistry(port, name) {
    try {
...
// ── Detection helpers ───────────────────────────────────────────────
export function detectPort(repo, _task) {
 ...
 */
export function detectBackend(repo) {
...
export function healthCheck(port, retries = 3, delay = 3) {
    for (let i = 0; i < retries; i++) {
        try {
            // Keep execSync: uses shell quoting and redirect (2>/dev/null)
            const code = execSync("curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:" +
                port +
                "/ 2>/dev/null", { timeout: 10000 })
                .toString()
                .trim();
...
export function findExistingService(_port, slug, repo) {
    try {
...
function ensureExpressStatic(repo) {
    ...
    if (!src.includes("import path from") &&
...
// ── Slug / naming helpers ───────────────────────────────────────────
export function slugify(name, maxLen = 30) {
    let slug = name
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "");
...
export function autoDeployProject(projectId, projectName, repo, task) {
    ...
    if (existing) {
        try {
            ...
            console.log('[auto-deploy] Existing service "' +
                existing +
                '" for repo ' +
                repo +
                " \u2014 restarting");
    ...
    try {
        ...
        if (!healthy) {
            console.error("[auto-deploy] Healthcheck FAILED: " +
                serviceName +
                " port " +
                port +
                " - rolling back");
